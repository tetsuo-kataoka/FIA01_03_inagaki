...
const Lists = ({ uid, clearTime, clearWeek }) => {
  ...
  const handleAddSubmit = async (
    ...
    let newMemo = leftMemo !== "" ? leftMemo : memo;
  ...
  // 一歩を押したら今のは登録して新しいタスクを文字列分解して作成
  const handleSplitSubmit = async (e) => {
    const memoArray = memo.split(/[＊\*]/);
    let left_memo = "";
    let sub_title = "";
    let sub_memo = "";
    memoArray.map(async (str, index) => {
      if (index == 0) {
        // 元の文書で*以前を設定する
        left_memo = str;
      } else {
    ...
    // 切り取ったもののみ更新することを考えてみたが、ちょっとそのままにしてみる
    await handleAddSubmit(e); //{ leftMemo: memo }
    // 切り出したデータでダイアログを起動
    handleClickOpen(
      list,
      userId,
          ...
          <Grid container>
              ...
              <Grid container justifyContent="flex-start" spacing={0}>
                ...
                <Grid item>
                  <IconButton
                    aria-label="waiting"
                    onClick={(e) =>
                      handleAddSubmit(e, { specificListName: "waiting" })
                    }
                    size="small"
                    color="primary"
                  >
            ...
            <Grid item xs={2}>
              <Grid container justifyContent="center">
                <IconButton
                  onClick={(e) =>
                    handleAddSubmit(e, { specificListName: "done" })
                  }
                  size="small"
                  color="primary"
                  aria-label="done"
